...
static void get_guest_descriptor(struct vcpu_request *requ, int base,
	int limit);
...
void get_guest_state_pcpu(void)
{
	...
	case general_purpose_registers:
		get_gpr_state((unsigned char *)&req->guest_data.g_gprs,
			&req->guest_data_sz);
		return;
...
{
	struct x86_regs *p_regs = &req->guest_data.g_states;
	...
	req->guest_data_sz = sizeof(struct x86_regs);
}

static void get_msr(struct vcpu_request *req)
{
	unsigned int msr = req->query_gstate_param;
...
/*
 *Get base, limit, selector and rights of all segment registers:
 *cs, ds, ss, es, fs, gs
 **/
static void get_sreg_state(struct vcpu_request *req)
{
...
static void get_guest_descriptor(struct vcpu_request *req, int base, int limit)
{
	req->guest_data.dtr.base = vmcs_readl(base);
	req->guest_data.dtr.limit = vmcs_read32(limit);
